...
def render(data, output_path="cold-start-report.html"):
    ...
    for t, color in type_colors.items():
        ...
        commit_bars += f'<div class="bar-row"><span class="bar-label">{t}</span><div class="bar-track"><div class="bar-fill" style="width:{pct}%;background:{color}"></div></div><span class="bar-count">{count} <span class="muted">({pct}%)</span></span></div>'
    ...
    for z in ai.get("danger_zones", []):
        ...
        danger_html += f'<div class="danger-card" style="border-color:{severity_color}25;background:{severity_color}08"><div class="danger-label" style="color:{severity_color}">⚠ {z.get("zone","Risk")}</div><div class="danger-warning">{z.get("warning","")}</div><div class="danger-files">{files_html}</div></div>'
    ...
    if shock:
        shock_block = f"""
<div class="shock-block">
  <div class="shock-label">⚡ INSIGHT YOU DIDN'T EXPECT</div>
  <div class="shock-text">{shock}</div>
</div>"""
...
.shock-block{{background:linear-gradient(135deg,#1a1200,#0e1318);border:1px solid #fbbf2440;border-radius:12px;padding:20px 24px;margin:0 0 24px}}
.shock-label{{font-size:10px;color:#fbbf24;letter-spacing:.2em;text-transform:uppercase;margin-bottom:8px}}
.shock-text{{font-family:'Syne',sans-serif;font-size:15px;font-weight:700;color:#fff;line-height:1.5}}
.purpose-block{{background:linear-gradient(135deg,#0d1e1a,#0e1318);border:1px solid #00d4aa25;border-radius:12px;padding:24px 28px;margin-bottom:24px}}
...
.bar-row{{display:grid;grid-template-columns:90px 1fr 80px;gap:8px;align-items:center;margin-bottom:8px}}
...
.pair-files{{display:flex;align-items:center;gap:8px;margin-bottom:4px;flex-wrap:wrap}}
...
.danger-card{{border-radius:8px;padding:14px 16px;margin-bottom:8px}}
.danger-label{{font-size:12px;font-weight:600;margin-bottom:6px}}
.danger-warning{{color:#c8dce8;font-size:12px;line-height:1.6;margin-bottom:8px}}
...
.commit-dot{{width:7px;height:7px;border-radius:50%;display:inline-block;flex-shrink:0}}
...
<body>
<div class="page">
<header style="border-bottom:1px solid #1e2a35;padding-bottom:28px;margin-bottom:32px">
...
<div class="stats-row">
  ...
  <div class="stat-pill"><span class="stat-value">{len(ai.get("danger_zones",[]))}</span><span class="stat-label">danger zones</span></div>
</div>
...
<div class="grid-2">
  <div class="card"><div class="card-title">Danger Zones</div>{danger_html or '<div class="muted" style="font-size:11px;padding:8px 0">No danger zones detected.</div>'}</div>
  <div class="card"><div class="card-title">Implicit Coupling Map</div>{cochange_html or '<div class="muted" style="font-size:11px;padding:8px 0">No coupling pairs detected.</div>'}</div>
</div>
...
<div class="grid-2">
  ...
  <div class="card">
    ...
    <div style="margin-top:24px"><div class="card-title">Top Contributors</div>{contributors_html}</div>
  </div>
</div>

<div class="card" style="margin-bottom:20px">
  <div class="card-title">Recent Commit Timeline</div>{commits_html}
</div>

<footer class="footer">
  <span>context-collapse · Cold Start Pack</span>
  <span>Understand any codebase in 30 minutes — not 3 days.</span>
</footer>
</div>
</body>
</html>"""
